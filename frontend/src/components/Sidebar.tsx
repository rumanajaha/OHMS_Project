...
import {
  LayoutDashboard, Users, LogOut, Menu, X, CheckSquare, Settings, Network, Grid, FileText, Bell
} from 'lucide-react';
...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      <button className="mobile-sidebar-toggle btn btn-ghost" onClick={() => setIsOpen(!isOpen)} style={{ position: 'fixed', top: '1rem', left: '1rem', zIndex: 50, display: 'none' }}>
        {isOpen ? <X /> : <Menu />}
      </button>

      <aside className={`layout-sidebar ${isOpen ? 'open' : ''}`}>
        <div style={{ padding: '2rem 1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ width: '36px', height: '36px', background: 'var(--primary)', borderRadius: '10px', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Network size={20} />
          </div>
          <span style={{ fontSize: '1.125rem', fontFamily: 'Poppins, sans-serif', fontWeight: 600, color: 'var(--primary)' }}>OrgSys</span>
        </div>

        <nav style={{ padding: '0 1rem', display: 'flex', flexDirection: 'column', flex: 1, marginTop: '1rem' }}>
          <div style={{ fontSize: '0.75rem', textTransform: 'uppercase', color: 'var(--text-light)', fontWeight: 600, letterSpacing: '0.05em', padding: '0 0.5rem', marginBottom: '0.5rem' }}>
            Main Menu
          </div>
          {links.map((link) => (
        ...
        <div style={{ padding: '1.5rem 1rem' }}>
          <NavLink to={`/${user.role.toLowerCase()}/profile`} className={({ isActive }) => `sidebar-link ${isActive ? 'active' : ''}`}>
            <span className="sidebar-link-icon"><Settings size={18} /></span>
            <span>Settings</span>
          </NavLink>
          <button onClick={handleLogout} className="sidebar-link" style={{ width: '100%', border: 'none', background: 'transparent', textAlign: 'left', cursor: 'pointer' }}>
      ...
      {isOpen && (
        <div onClick={() => setIsOpen(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(31, 41, 51, 0.4)', backdropFilter: 'blur(4px)', zIndex: 30 }} />
      )}
    </>
  );
};
