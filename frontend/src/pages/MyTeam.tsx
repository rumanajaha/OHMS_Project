  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
        ...
        {/* Tree Connection Line (Horizontal) */}
        {myTeam.length > 1 && (
          <div style={{ width: `calc(100% - ${100 / myTeam.length}%)`, height: '2px', background: 'var(--border-color)', position: 'relative' }}>
             {myTeam.map((_, i) => (
                <div key={i} style={{ position: 'absolute', top: 0, left: `${(100 / (myTeam.length - 1)) * i}%`, width: '2px', height: '20px', background: 'var(--border-color)', transform: 'translateY(0%)' }} />
             ))}
          </div>
        )}
        
        {/* If only 1 member, horizontal line is not needed, just vertical */}
        {myTeam.length === 1 && (
          <div style={{ width: '2px', height: '20px', background: 'var(--border-color)', margin: '0 auto' }} />
        )}

        {/* Team Members Nodes (Bottom) */}
