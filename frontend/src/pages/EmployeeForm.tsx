...
export const EmployeeForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    status: existingEmp?.status || 'Active' as EmployeeStatus,
  ...
  const managerName = formData.managerId ? employees.find(e => e.id === formData.managerId)?.firstName : 'None';

  return (
    <div className="animate-fade-in" style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ marginBottom: '2rem' }}>
        <h1 className="h1">{isEditMode ? 'Edit Employee' : 'Add New Employee'}</h1>
        <p className="text-muted">Fill out the information below to {isEditMode ? 'update' : 'create'} an employee profile.</p>
      </div>
          ...
          <div className="animate-fade-in">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', background: 'var(--bg-subtle)', padding: '1.5rem', borderRadius: 'var(--radius-md)', marginBottom: '2rem' }}>
              ...
              <div>
                ...
                <p><span className={`badge badge-${formData.status === 'Active' ? 'success' : 'warning'}`}>{formData.status}</span></p>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
              ...
              <button type="button" className="btn btn-primary" onClick={handleSave} disabled={isSaving}>
                {isSaving ? <span className="spinner" style={{ width: '16px', height: '16px' }} /> : (isEditMode ? 'Save Changes' : 'Confirm & Create')}
              </button>
            </div>
          </div>
        )}
