      ...
      <div className="card" style={{ padding: '0', overflow: 'hidden' }}>
         ...
         <div style={{ padding: '0 2rem 2rem 2rem', position: 'relative' }}>
            <div style={{ 
              ...
              fontSize: '2.5rem', fontWeight: 600, border: '4px solid var(--bg-surface)', 
              marginTop: '-50px', marginBottom: '1rem', boxShadow: 'var(--shadow-sm)' 
            }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
               <div>
                  <h1 className="h1" style={{ marginBottom: '0.25rem' }}>{employee.firstName} {employee.lastName}</h1>
                  <p className="text-lg text-muted">{employee.designation}</p>
               </div>
               <span className={`badge badge-${employee.status === 'Active' ? 'success' : 'neutral'}`}>{employee.status}</span>
            </div>
               ...
               <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                 ...
                 <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                   ...
                   <div>
                     ...
                     <p className="font-medium">{manager ? `${manager.firstName} ${manager.lastName}` : 'N/A'}</p>
                   </div>
                 </div>
                 <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
            ...
            <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '2rem', marginTop: '2rem', display: 'flex', gap: '1rem' }}>
                {user?.role !== 'Employee' && (
                  <button className="btn btn-primary" onClick={() => navigate(`/admin/employees/edit/${employee.id}`)}>Edit Record</button>
                )}
