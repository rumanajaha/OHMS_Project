...
export const ManagerDashboard: React.FC = () => {
  ...
  const myTeam = employees.filter(e => e.managerId === user?.id || e.departmentId === user?.departmentId);
          ...
          <div style={{ padding: '1rem 1.5rem', maxHeight: '350px', overflowY: 'auto' }}>
            {filteredTeam.length === 0 ? (
              ...
              filteredTeam.slice(0, 5).map((member) => (
                <div key={member.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 0', borderBottom: '1px solid var(--border-color)', cursor: 'pointer' }} onClick={() => navigate(`/admin/employees/view/${member.id}`)}>
        ...
        <div className="card" style={{ padding: 0 }}>
          ...
          <div style={{ padding: '1.5rem', maxHeight: '350px', overflowY: 'auto' }}>
            ...
            {pendingTasks.slice(0, 5).map(task => (
              <div key={task.id} style={{ padding: '1rem', background: 'var(--bg-main)', borderRadius: 'var(--radius-md)', marginBottom: '1rem', borderLeft: `3px solid ${task.priority === 'High' ? 'var(--danger)' : task.priority === 'Medium' ? 'var(--warning)' : 'var(--success)'}`, cursor: 'pointer' }} onClick={() => navigate(`/manager/tasks/${task.id}`)}>
