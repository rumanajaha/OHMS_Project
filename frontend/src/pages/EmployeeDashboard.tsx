...
import { User, Users, CheckSquare, Bell, Search, Activity, UserX, UserCheck, ChevronRight } from 'lucide-react';
...
export const EmployeeDashboard: React.FC = () => {
  ...
  const managerName = managerData ? `${managerData.firstName} ${managerData.lastName}` : 'System Assigned';

  const myTeam = employees.filter(e => e.departmentId === currentUserData?.departmentId && e.id !== user?.id);
  
  const myTasks = tasks.filter(t => t.assignee.includes(user?.name?.split(' ')[0] || '') || t.assignee === 'You');
  ...
  // Mocking attendance stats for UI completeness
    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p className="text-muted text-sm">Welcome back, {user?.name?.split(' ')[0] || ''}. Here's your personal overview.</p>
        </div>

        <div className="search-bar" style={{ display: 'flex', alignItems: 'center', background: 'var(--bg-surface)', padding: '0.75rem 1.25rem', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border-color)', width: '300px' }}>
      ...
      <div className="grid-cards" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))' }}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', cursor: 'pointer' }} onClick={() => navigate('/employee/tasks')}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', cursor: 'pointer' }} onClick={() => navigate('/employee/notifications')}>
      ...
      <div className="grid-cards" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
        <div className="card" style={{ padding: '1.25rem', borderLeft: '4px solid var(--success)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}><UserCheck size={16} className="text-success" /><span className="text-sm font-medium">Regular (Present)</span></div>
          <div className="h2">{regularCount}</div>
        </div>
        <div className="card" style={{ padding: '1.25rem', borderLeft: '4px solid var(--warning)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}><Activity size={16} className="text-warning" /><span className="text-sm font-medium">Irregular (Late/Half)</span></div>
          <div className="h2">{irregularCount}</div>
        </div>
        <div className="card" style={{ padding: '1.25rem', borderLeft: '4px solid var(--danger)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}><UserX size={16} className="text-danger" /><span className="text-sm font-medium">On Leave</span></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '2rem' }}>
          ...
          <div style={{ padding: '1rem 1.5rem', maxHeight: '350px', overflowY: 'auto' }}>
            {myTeam.length === 0 ? (
              ...
              myTeam.slice(0, 5).map((member) => (
                <div key={member.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 0', borderBottom: '1px solid var(--border-color)' }}>
        ...
        <div className="card" style={{ padding: 0 }}>
          <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button className="btn btn-ghost" style={{ fontSize: '0.75rem', padding: '0.25rem' }} onClick={() => navigate('/employee/tasks')}>View All</button>
          </div>
          <div style={{ padding: '1.5rem', maxHeight: '350px', overflowY: 'auto' }}>
            ...
            {filteredTasks.map(task => (
              <div key={task.id} style={{ padding: '1rem', background: 'var(--bg-main)', borderRadius: 'var(--radius-md)', marginBottom: '1rem', borderLeft: `3px solid ${task.priority === 'High' ? 'var(--danger)' : task.priority === 'Medium' ? 'var(--warning)' : 'var(--success)'}`, cursor: 'pointer' }} onClick={() => navigate(`/employee/tasks/${task.id}`)}>
