...
public class EmployeeController {
    ...
    @GetMapping("/search")
    public List<EmployeeDto> searchEmployees(@RequestParam(required = false) String name, @RequestParam(required = false) Long departmentId, @RequestParam(required = false) Long positionId,@RequestParam(required = false) Long managerId, @RequestParam(required = false) EmployeeStatus status){
        return employeeService.searchEmployees(name,departmentId, positionId,managerId, status);
    }
}
